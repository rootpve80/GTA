...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('pycore_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('[PYCORE.CLOUD]')
...
ALLOWED_SUPERS = {1372237657207345183, 1279500219154956419} # Superuser can add/remove admins and update/restart bot.py
...
WATERMARK = "PYCORE CLOUD VPS Service"
WELCOME_MESSAGE = "Welcome To PYCORE CLOUD! Get Started With Us!"
MAX_VPS_PER_USER = int(os.getenv('MAX_VPS_PER_USER', '3'))
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'bridge')
MAX_CONTAINERS = int(os.getenv('MAX_CONTAINERS', '100'))
DB_FILE = 'pycore.db'
BACKUP_FILE = 'pycore_backup.pkl'
...
class pycoreBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            # === أهم إضافة: مزامنة أوامر السلاش ===
            GUILD_ID = os.getenv("GUILD_ID")  # اختياري لتسريع الظهور في سيرفر واحد
            if GUILD_ID:
                guild = discord.Object(id=int(GUILD_ID))
                # لو عندك أوامر Global بالفعل: نزّل نسخة للسيرفر ده
                self.tree.copy_global_to(guild=guild)
                await self.tree.sync(guild=guild)     # يظهر فورًا في السيرفر المحدد
                logger.info(f"Synced commands to guild {GUILD_ID}")
            else:
                # Global sync (قد تتأخر أول مرة دقائق)
                await self.tree.sync()
                logger.info("Synced global application commands")

        except Exception as e:
            logger.error(f"Failed to initialize Docker client or sync commands: {e}")
            self.docker_client = None
...
async def build_custom_image(vps_id, username, root_password, user_password, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        # Build the image
        image_tag = f"pycore/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    """Enhanced container setup with PYCORE CLOUD customization"""
    try:
        ...
        # Set PYCORE CLOUD customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up PYCORE CLOUD customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up PYCORE CLOUD customization...")
        ...
        hostname_cmd = f"echo 'pycore-{vps_id}' > /etc/hostname && hostname pycore-{vps_id}"
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ PYCORE CLOUD VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ PYCORE CLOUD VPS setup completed successfully!")
...
bot = pycoreBot(command_prefix=PREFIX, intents=intents, help_command=None)
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 PYCORE CLOUD VPS Bot Commands", color=discord.Color.blue())
...
`/vps_restart <vps_id>` - Restart your VPS
`/vps_stop <vps_id>` - Stop your VPS
`/vps_start <vps_id>` - Start your VPS
`/vps_token <vps_id>` - Get your VPS access token
`/vps_info <vps_id>` - Show detailed VPS information
...
""", inline=False)
        ...
        await ctx.send("❌ An error occurred while processing your request.")
@bot.hybrid_command(name='restart_bot', description='Restart the bot (Admin only)')
async def restart_bot(ctx):
    ...
    try:
        ...
        logger.info(f"Bot restart initiated by {ctx.author} ({ctx.author.id})")
        await bot.close()
        os.execv(sys.executable, ['python'] + sys.argv)
    except Exception as e:
        logger.error(f"Error restarting bot: {e}")
        await ctx.send("❌ An error occurred while trying to restart the bot.")
@bot.hybrid_command(name='show_botpy', description='show bot.py contect (Owner only)')
async def show_botpy(ctx):
    """Show the bot.py content (Owner only)"""
    if ctx.author.id not in ALLOWED_SUPERS:
    ...
    try:
        ...
        if len(code) > 1900:
            ...
            with open('bot_code.txt', 'w') as temp_file:
                temp_file.write(code)
            await ctx.send("📄 bot.py content is too long, sending as a file:", file=discord.File('bot_code.txt'))
            os.remove('bot_code.txt')
        else:
            await ctx.send(f"```python\n{code}\n```")
    except Exception as e:
        logger.error(f"Error showing bot.py content: {e}")
        await ctx.send("❌ An error occurred while retrieving bot.py content.")

@bot.hybrid_command(
    name='update_botpy',
    description="Update the bot.py code and .env and all files in the bot folder (ALLOWED_SUPERS only)"
)
@app_commands.describe(
    method="Update method: 'git' or 'py file to replace'",
    python_url="URL to the new bot.py file (if method is 'py file to replace')",
)
async def update_botpy(
    ctx: commands.Context,
    method: Literal['git', 'py file to replace'],
    python_url: Optional[str] = None,   # <-- add this
):
    await ctx.defer()  # optional, helps for slash use

    if method == 'py file to replace':
        if not python_url:
            return await ctx.reply("You chose 'py file to replace' but didn't provide a python_url.")
        # do your replace flow using python_url
        return await ctx.reply(f"Replacing bot.py from: {python_url}")

    # method == 'git'
    # do your git update flow here
    await ctx.reply("Updating via git…")
    """Update the bot.py code and .env and all files in the bot folder (ALLOWED_SUPERS only)"""
    if ctx.author.id not in ALLOWED_SUPERS:
        await ctx.send("❌ You are not allowed to use this command.", ephemeral=True)
        return
    try:
        if method.lower() == 'git':
            await ctx.send("🔄 Updating bot via git...")
            process = await asyncio.create_subprocess_exec(
                "git", "pull",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            ...
            await ctx.send("❌ Unsupported update method. Use 'git'.", ephemeral=True)
    except Exception as e:
        logger.error(f"Error updating bot.py: {e}")
        await ctx.send(f"❌ An error occurred while updating: {str(e)}", ephemeral=True)
    try:
        if method.lower() == 'py file to replace':
            await ctx.send("🔄 Updating bot.py from provided URL...")
            python_url = python_url.strip()
            async with bot.session.get(python_url) as response:
                if response.status != 200:
                    raise Exception(f"Failed to download file: HTTP {response.status}")
                new_code = await response.text()
            with open('bot.py', 'w') as f:
                f.write(new_code)
            await ctx.send("✅ bot.py updated successfully from the provided URL.")
    except Exception as e:
        logger.error(f"Error updating bot.py from URL: {e}")
        await ctx.send(f"❌ An error occurred while updating from URL: {str(e)}", ephemeral=True)
...
async def system_info(ctx):
    ...
    try:
        ...
        docker_version = bot.docker_client.version()['Version'] if bot.docker_client else "N/A"
        
        embed = discord.Embed(title="🖥️ System Information", color=discord.Color.green())
        embed.add_field(name="CPU", value=cpu_info or "N/A", inline=False)
        embed.add_field(name="Memory", value=f"{mem.total / (1024 ** 3):.2f} GB", inline=False)
        embed.add_field(name="Disk", value=f"{disk.total / (1024 ** 3):.2f} GB", inline=False)
        embed.add_field(name="System Uptime", value=boot_time, inline=False)
        embed.add_field(name="Docker Version", value=docker_version, inline=False)
        
        await ctx.send(embed=embed) 
    except Exception as e:
        logger.error(f"Error in system_info command: {e}")
@bot.hybrid_command(name="add_admin", description="Add a new admin permanently (Superusers only)")
@app_commands.describe(user="User to make admin")
async def add_admin(ctx: commands.Context, user: discord.User):
    """Add a new admin user (Superusers only)"""
    # Allow only the specified IDs
    if ctx.author.id not in ALLOWED_SUPERS:
    ...
    # Perform the addition (ensure ctx.bot.db.add_admin exists)
    try:
        ctx.bot.db.add_admin(user.id)
        msg = f"✅ {user.mention} has been added as a permanent admin."
    except Exception as e:
        msg = f"⚠️ An error occurred while adding: `{e}`"

    if ctx.interaction and not ctx.interaction.response.is_done():
        await ctx.interaction.response.send_message(msg, ephemeral=True)
    else:
        await ctx.reply(msg, mention_author=False)
@bot.hybrid_command(name="remove_admin", description="Remove an admin (Superusers only)")
@app_commands.describe(user="User to remove from admin")
async def remove_admin(ctx: commands.Context, user: discord.User):
    """Remove an admin user (Superusers only)"""
    # Allow only the specified IDs
    if ctx.author.id not in ALLOWED_SUPERS:
        msg = "❌ You are not allowed to use this command."
        if ctx.interaction and not ctx.interaction.response.is_done():
    ...
    # Perform the removal (ensure ctx.bot.db.remove_admin exists)
    try:
        ctx.bot.db.remove_admin(user.id)
        msg = f"✅ {user.mention} has been removed from admins."
    except Exception as e:
        msg = f"⚠️ An error occurred while removing: `{e}`"
...
@app_commands.describe(
    ...
    use_custom_image="Use custom PYCORE CLOUD image (recommended)",
...
async def create_vps_command(
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating PYCORE CLOUD VPS instance... This may take a few minutes.")
        ...
        def _run_container(image: str, custom: bool):
            common = dict(
                ...
                volumes={
                    f'pycore-{vps_id}': {'bind': '/data', 'mode': 'rw'},
        ...
        await status_msg.edit(content="🔧 Container created. Setting up PYCORE CLOUD environment...")
        ...
        # إخطار + Embed
        try:
            embed = discord.Embed(title="🎉 PYCORE CLOUD VPS Creation Successful", color=discord.Color.green())
            embed.image.url = "https://i.imgur.com/fJTPMM9.png" 
            ...
            embed.add_field(name="🧠 VPS Token", value=f"||{token}||", inline=False)
            embed.add_field(
                name="ℹ️ Note",
                value=("This is a PYCORE CLOUD VPS instance. You can install and configure additional packages as needed."),
                inline=False
            )
            embed.add_field(
            ...
            await status_msg.edit(
                content=f"✅ PYCORE CLOUD VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details."
            )
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your PYCORE CLOUD VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All PYCORE CLOUD VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"🧹 Deleting PYCORE CLOUD VPS `{vps_id}`... Please wait ⏳", ephemeral=True)

        container_id = vps.get("container_id")
        image_name = f"pycore/{vps_id.lower()}:latest"
        volume_name = f"pycore-{vps_id}"
        ...
        try:
            paths = [
                f"pycore-temp/{vps_id}",
                f"migrations/{vps_id}",
                f"pycore_backup_{vps_id}.pkl",
        ...
        # ---- Final confirmation
        embed = discord.Embed(title=f"🧹 PYCORE CLOUD VPS {vps_id} Deleted", color=discord.Color.red())
...
@bot.hybrid_command(name='stop_vps', description='stop your VPS instance')
@app_commands.describe(
    vps_id="ID of the VPS to stop"
)
async def stop_vps(ctx, vps_id: str):
    """Stop a VPS instance owned by the user"""
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        ...
        try:
            ...
            await ctx.send(f"✅ VPS `{vps_id}` has been stopped successfully!", ephemeral=True)
        except docker.errors.NotFound:
            await ctx.send("❌ VPS instance not found or is no longer available.", ephemeral=True)
        except Exception as e:
            logger.error(f"Error stopping VPS {vps_id}: {e}")
            await ctx.send(f"❌ An error occurred while stopping the VPS: {str(e)}", ephemeral=True)

    except Exception as e:
        logger.error(f"Error in stop_vps: {e}")
        await ctx.send(f"❌ Error processing your request: {str(e)}", ephemeral=True)
        await asyncio.sleep(1.4)
        print("\n\n")
@commands.hybrid_command(name="start_vps", description="Start your VPS instance")
@app_commands.describe(vps_id="ID of the VPS to start")
async def start_vps(ctx: commands.Context, vps_id: str):
    """Start a VPS instance owned by the user."""
    # IMPORTANT: sending ephemeral messages only works on interactions;
    # ctx.send(ephemeral=True) is okay for hybrid if invoked as slash.
    try:
        # --- fetch VPS record ---
        token, vps = bot.db.get_vps_by_id(vps_id)  # adjust if your DB call is async
        if not vps:
            await ctx.send("❌ VPS not found!", ephemeral=True)
            return

        if vps.get("created_by") != str(ctx.author.id) and not has_admin_role(ctx):
            await ctx.send("❌ You don't have access to this VPS.", ephemeral=True)
            return

        container_id = vps.get("container_id")
        if not container_id:
            await ctx.send("❌ VPS record is missing container_id.", ephemeral=True)
            return

        # --- interact with Docker (avoid blocking the event loop) ---
        loop = asyncio.get_running_loop()
        client: docker.DockerClient = bot.docker_client

        def _start_container(cid: str) -> Tuple[str, str]:
            # returns (status, message)
            try:
                container = client.containers.get(cid)
            except docker.errors.NotFound:
                return ("not_found", "❌ VPS instance not found or is no longer available.")

            # Refresh status (in case it's stale)
            container.reload()
            if container.status == "running":
                return ("already_running", "❌ VPS is already running!")

            try:
                container.start()
                container.reload()
                return ("started", "✅ VPS has been started successfully!")
            except docker.errors.APIError as e:
                return ("docker_error", f"❌ Docker API error while starting the VPS: {e.explanation if hasattr(e, 'explanation') else str(e)}")
            except Exception as e:
                return ("error", f"❌ An unexpected error occurred while starting the VPS: {str(e)}")

        status, msg = await loop.run_in_executor(None, _start_container, container_id)

        # --- update DB if we actually started it ---
        if status == "started":
            try:
                bot.db.update_vps(token, {"status": "running"})
            except Exception as e:
                # Log but don't fail the user-facing flow
                logger.error(f"DB update failed for VPS {vps_id}: {e}")

            await ctx.send(f"{msg} (`{vps_id}`)", ephemeral=True)
        elif status in ("already_running", "not_found"):
            await ctx.send(msg, ephemeral=True)
        else:
            await ctx.send(msg, ephemeral=True)

    except Exception as e:
        logger.error(f"Error in start_vps: {e}", exc_info=True)
        await ctx.send(f"❌ Error processing your request: {str(e)}", ephemeral=True)
@bot.hybrid_command(name='restart_vps', description='Restart your VPS instance')
@app_commands.describe(
    vps_id="ID of the VPS to restart"
)
async def restart_vps(ctx, vps_id: str):
    """Restart a VPS instance owned by the user"""
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or (vps["created_by"] != str(ctx.author.id) and not has_admin_role(ctx)):
            await ctx.send("❌ VPS not found or you don't have access to it!", ephemeral=True)
        ...
        try:
            ...
            await ctx.send(f"✅ VPS `{vps_id}` has been restarted successfully!", ephemeral=True)
        except docker.errors.NotFound:
            await ctx.send("❌ VPS instance not found or is no longer available.", ephemeral=True)
        except Exception as e:
            logger.error(f"Error restarting VPS {vps_id}: {e}")
            await ctx.send(f"❌ An error occurred while restarting the VPS: {str(e)}", ephemeral=True)

    except Exception as e:
        logger.error(f"Error in restart_vps: {e}")
        await ctx.send(f"❌ Error processing your request: {str(e)}", ephemeral=True)
        print("\n\n")

@bot.hybrid_command(name='vps_info', description='Get detailed info about a VPS (Admin Only)')
@app_commands.describe(
    vps_id="ID of the VPS to get info about"
)
async def vps_info(ctx, vps_id: str):
    """Get detailed info about a VPS (Admin Only)"""
    if not has_admin_role(ctx):
        await ctx.send("❌ You must be an admin to use this command!", ephemeral=True)
        return
    try:
        ...
        embed = discord.Embed(title=f"VPS Info: {vps_id}", color=discord.Color.blue())
        for key, value in vps.items():
            embed.add_field(name=key.replace("_", " ").title(), value=str(value), inline=False)

        await ctx.send(embed=embed)
    except Exception as e:
        logger.error(f"Error in vps_info: {e}")
        await ctx.send(f"❌ Error retrieving VPS info: {str(e)}", ephemeral=True)
@bot.hybrid_command(name='vps_token', description='Get the access token for your VPS')
@app_commands.describe(
    vps_id="ID of the VPS to get the token for"
)
async def vps_token(ctx, vps_id: str):
    """Get the access token for your VPS"""
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or (vps["created_by"] != str(ctx.author.id) and not has_admin_role(ctx)):
            await ctx.send("❌ VPS not found or you don't have access to it!", ephemeral=True)
        ...
        await ctx.send(f"🔑 The access token for VPS `{vps_id}` is:\n||{token}||", ephemeral=True)
    except Exception as e:
        logger.error(f"Error in vps_token: {e}")
        await ctx.send(f"❌ Error retrieving VPS token: {str(e)}", ephemeral=True)
@bot.hybrid_command(name='set_vps_password', description='Set a new SSH password for your VPS')
@app_commands.describe(
    vps_id="ID of the VPS to set the password for",
    new_password="New SSH password to set"
)
async def set_vps_password(ctx, vps_id: str, new_password: str):
    """Set a new SSH password for your VPS"""
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or (vps["created_by"] != str(ctx.author.id) and not has_admin_role(ctx)):
            await ctx.send("❌ VPS not found or you don't have access to it!", ephemeral=True)
        ...
        try:
            ...
            exec_cmd = container.exec_run(
                f"bash -lc 'echo \"{vps['username']}:{new_password}\" | chpasswd'",
                stdout=True, stderr=True
            )
            if exec_cmd.exit_code != 0:
                raise Exception(exec_cmd.output.decode())

            bot.db.update_vps(token, {"password": new_password})
            await ctx.send(f"✅ SSH password for VPS `{vps_id}` has been updated successfully!", ephemeral=True)
        except docker.errors.NotFound:
            await ctx.send("❌ VPS instance not found or is no longer available.", ephemeral=True)
        except Exception as e:
            logger.error(f"Error setting password for VPS {vps_id}: {e}")
            await ctx.send(f"❌ An error occurred while setting the password: {str(e)}", ephemeral=True)

    except Exception as e:
        logger.error(f"Error in set_vps_password: {e}")
        await ctx.send(f"❌ Error processing your request: {str(e)}", ephemeral=True)
        print("\n\n")
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="PYCORE CLOUD VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your PYCORE CLOUD VPS
...
""".format(username=vps["username"]), inline=False)
        ...
        await ctx.send("✅ Connection details sent to your DMs! Use the Tmate command to connect to your PYCORE CLOUD VPS.", ephemeral=True)
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="PYCORE CLOUD System Statistics", color=discord.Color.blue())
...
@bot.hybrid_command(name='system_info1', description='Show detailed system information (Admin only)')
async def system_info(ctx):
    """Show detailed system information (Admin only)"""
    if not has_admin_role(ctx):
        await ctx.send("❌ You must be an admin to use this command!", ephemeral=True)
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your PYCORE CLOUD VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="PYCORE CLOUD Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling PYCORE CLOUD bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "pycore-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"PYCORE CLOUD VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"PYCORE CLOUD VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ PYCORE CLOUD VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"PYCORE CLOUD VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ PYCORE CLOUD VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"PYCORE CLOUD VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("✅ PYCORE CLOUD VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling PYCORE CLOUD VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'pycore-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"PYCORE CLOUD VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="✅ PYCORE CLOUD VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling PYCORE CLOUD VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ PYCORE CLOUD VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="PYCORE CLOUD VPS Transferred to You", color=discord.Color.green())
...
@bot.hybrid_command(name='ssh_regen', description='Regenerate SSH Command for a VPS (Admin Only)')
@app_commands.describe(
    vps_id="ID of the VPS to regenerate SSH command for"
)
async def ssh_regen(ctx, vps_id: str):
    """Regenerate SSH Command for a VPS (Admin Only)"""
    if not has_admin_role(ctx):
        await ctx.send("❌ You must be an admin to use this command!", ephemeral=True)
        return
    try:
        ...
        try:
            ...
            ssh_session_line = await capture_ssh_session_line(exec_cmd)
            if not ssh_session_line:
                await ctx.send("❌ Failed to generate new SSH session!", ephemeral=True)
                return

            bot.db.update_vps(token, {'tmate_session': ssh_session_line})

            await ctx.send(
                f"✅ New SSH session generated for VPS {vps_id}:\n```{ssh_session_line}```",
                ephemeral=True
            )
            ...
            try:
                owner = await bot.fetch_user(int(vps["created_by"]))
                embed = discord.Embed(
                    title=f"PYCORE CLOUD VPS SSH Regenerated - {vps_id}",
                    color=discord.Color.blue()
                )
                embed.add_field(name="New SSH Session", value=f"```{ssh_session_line}```", inline=False)
                await owner.send(embed=embed)
            except Exception:
                # ما توقفش الكوماند لو فشل إرسال DM
                pass

        except Exception as e:
            await ctx.send(f"❌ Error generating SSH session: {str(e)}", ephemeral=True)

    except Exception as e:  # ← ده كان ناقص
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed = discord.Embed(title=f"PYCORE CLOUD VPS Management - {vps_id}", color=discord.Color.blue())
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ PYCORE CLOUD VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
...
async def on_ready():
    ...
    # change BOT RPC randomly every 10 seconds
    active1 = f"Managing PYCORE CLOUD VPS(s)"
    active2 = f"Pycore Cloud Service Top 1"
    active3 = f"Hosting VPS on Cloud"
    active4 = f"Use /help for commands"
    active5 = f"Managing your Pycore VPS"
    activities = [active1, active2, active3, active4, active5] 
    while True:
...
if __name__ == "__main__":
    try:
        os.makedirs("pycore-temp", exist_ok=True)
