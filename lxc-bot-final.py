...
logger = logging.getLogger('snow_node_vps_bot')
...
# Embed creation functions with black theme and SNOW NODE branding
def create_embed(title, description="", color=0x1a1a1a):
    """Create a dark-themed embed with proper field length handling and SNOW NODE branding"""
    embed = discord.Embed(
        title=truncate_text(f"⭐ SNOW NODE - {title}", 256),
...
def dev_mode_check():
    async def predicate(ctx):
        if dev_mode and str(ctx.author.id) != str(MAIN_ADMIN_ID):
            raise commands.CheckFailure("🛠 DEVELOPMENT MODE ON — Please wait for maintenance.")
        return True
...
def is_admin():
    async def predicate(ctx):
        ...
        raise commands.CheckFailure(f"You need admin permissions to use this command. Contact SNOW NODE support.")
    return commands.check(predicate)
...
# Resource allocation limits for non-main admins
def check_resource_limits(ram, cpu, disk, user_id):
    """Check if resource allocation exceeds limits for non-main admins"""
    if str(user_id) == str(MAIN_ADMIN_ID):
        return True, ""  # Main admin has no limits
    
    if ram > 95:
        return False, "RAM allocation cannot exceed 95GB for admins"
    if cpu > 95:
        return False, "CPU allocation cannot exceed 95 cores for admins"
    if disk > 95:
        return False, "Disk allocation cannot exceed 95GB for admins"
...
async def get_or_create_vps_role(guild):
    ...
    role = discord.utils.get(guild.roles, name="SNOW NODE VPS User")
    if role:
    ...
    try:
        role = await guild.create_role(
            name="SNOW NODE VPS User",
            color=discord.Color.dark_purple(),
            reason="SNOW NODE VPS User role for bot management",
            permissions=discord.Permissions.none()
        )
        VPS_USER_ROLE_ID = role.id
        logger.info(f"Created SNOW NODE VPS User role: {role.name} (ID: {role.id})")
        return role
    except Exception as e:
        logger.error(f"Failed to create SNOW NODE VPS User role: {e}")
        return None
...
async def auto_heal_task():
    """Background task to auto-heal stopped containers and ensure system health"""
    while True:
        try:
            ...
            for user_id, vps_list in vps_data.items():
                for vps in vps_list:
                    if vps.get('status') == 'running' and not vps.get('suspended', False):
                        ...
                        actual_status = await get_container_status(container_name)
                        
                        if actual_status != 'Running':
                            logger.info(f"Auto-healing container {container_name} (status: {actual_status})")
                            try:
                                await execute_lxc(f"lxc start {container_name}")
                                logger.info(f"Successfully auto-healed {container_name}")
                            except Exception as e:
                                logger.error(f"Failed to auto-heal {container_name}: {e}")
            
            # Ensure tmate is installed in running containers
            for user_id, vps_list in vps_data.items():
                for vps in vps_list:
                    if vps.get('status') == 'running' and not vps.get('suspended', False):
                        container_name = vps['container_name']
                        try:
                            ...
                            if check_proc.returncode != 0:
                                logger.info(f"Installing tmate in {container_name}")
                            ...
                            logger.error(f"Error ensuring tmate in {container_name}: {e}")
            
            await asyncio.sleep(300)  # Run every 5 minutes
        except Exception as e:
            logger.error(f"Auto-heal task error: {e}")
            await asyncio.sleep(60)

# Anti-abuse monitoring
async def anti_abuse_monitor():
    """Monitor for abusive behavior and auto-suspend offending VPS"""
    while True:
        try:
            for user_id, vps_list in vps_data.items():
                for vps in vps_list:
                    if vps.get('status') == 'running' and not vps.get('suspended', False):
                        ...
                        # Check for high CPU miner behavior
                        cpu_usage = await get_container_cpu_pct(container_name)
                        if cpu_usage > 95:  # Very high sustained CPU usage
                            logger.warning(f"Potential miner detected in {container_name}: CPU {cpu_usage}%")
                            
                            # Check processes for mining indicators
                            try:
                                proc = await asyncio.create_subprocess_exec(
                                ...
                                mining_indicators = ['xmrig', 'cpuminer', 'minerd', 'ccminer', 'nicehash']
                                if any(indicator in processes for indicator in mining_indicators):
                                    # Confirm mining activity - suspend VPS
                                    ...
                                    vps['suspension_history'].append({
                                        'time': datetime.now().isoformat(),
                                        'reason': 'Mining/cryptocurrency activity detected',
                                        'by': 'SNOW NODE Anti-Abuse System'
                                    })
                                    save_data()
                                    ...
                                    try:
                                        ...
                                        embed = create_warning_embed(
                                            "🚨 Anti-Abuse Action",
                                            f"VPS `{container_name}` suspended for mining activity\n\n**Owner:** <@{user_id}>\n**CPU Usage:** {cpu_usage}%\n**Action:** Auto-suspended"
                                        )
                                        await main_admin.send(embed=embed)
                                    except Exception as e:
                                        logger.error(f"Failed to notify main admin: {e}")
                            except Exception as e:
                                logger.error(f"Error checking processes in {container_name}: {e}")
            
            await asyncio.sleep(300)  # Check every 5 minutes
        except Exception as e:
            logger.error(f"Anti-abuse monitor error: {e}")
            await asyncio.sleep(60)
...
async def rotate_presence():
    ...
    activities = [
        discord.Activity(type=discord.ActivityType.watching, name=f"{total_vps} VPS"),
        discord.Activity(type=discord.ActivityType.playing, name="COMPLETING DREAM"),
        discord.Activity(type=discord.ActivityType.watching, name="PROTECTING"),
        discord.Activity(type=discord.ActivityType.playing, name="MADE BY GG!")
    ]
    
    current_activity = activities[int(time.time()) % len(activities)]
...
async def on_ready():
    ...
    bot.loop.create_task(auto_heal_task())
    bot.loop.create_task(anti_abuse_monitor())
    logger.info("SNOW NODE Bot is ready! VPS monitoring started.")

@bot.event
async def on_message(message):
    # Check if main admin is mentioned
    if str(MAIN_ADMIN_ID) in message.content and f'<@{MAIN_ADMIN_ID}>' in message.content:
        try:
            await message.add_reaction('👑')
            await message.add_reaction('✨')
            # Note: Custom emoji would need actual emoji ID
            # await message.add_reaction('<:OWNER_GLOW:123456789012345678>')
        except Exception as e:
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        error_msg = str(error) if str(error) != "Admin required" else "You need admin permissions for this command. Contact SNOW NODE support."
        ...
        await ctx.send(embed=create_error_embed("System Error", "An unexpected error occurred. SNOW NODE support has been notified."))
...
async def ping(ctx):
    ...
    embed = create_success_embed("Pong!", f"SNOW NODE Bot latency: {latency}ms")
...
async def my_vps(ctx):
    ...
    if not vps_list:
        await ctx.send(embed=create_embed("No VPS Found", "You don't have any SNOW NODE VPS. Contact an admin to create one.", 0xff3366))
        return
    embed = create_info_embed("My SNOW NODE VPS", "")
    text = []
...
async def lxc_list(ctx):
    ...
    try:
        ...
        embed = create_info_embed("SNOW NODE LXC Containers List", result)
        await ctx.send(embed=embed)
    except Exception as e:
...
async def create_vps(ctx, ram: int, cpu: int, disk: int, user: discord.Member):
    ...
    # Check resource limits for non-main admins
    is_valid, error_msg = check_resource_limits(ram, cpu, disk, ctx.author.id)
    if not is_valid:
        await ctx.send(embed=create_error_embed("Resource Limit Exceeded", error_msg))
        return
    ...
    container_name = f"snow-node-vps-{user_id}-{vps_count}"
    ram_mb = ram * 1024

    await ctx.send(embed=create_info_embed("Creating SNOW NODE VPS", f"Deploying VPS for {user.mention}..."))
    ...
    try:
        ...
        if ctx.guild:
            ...
            if vps_role:
                try:
                    await user.add_roles(vps_role, reason="SNOW NODE VPS ownership granted")
                except discord.Forbidden:
                    logger.warning(f"Failed to assign SNOW NODE VPS role to {user.name}")

        # Create success embed for channel
        embed = create_success_embed("SNOW NODE VPS Created Successfully")
        ...
        try:
            dm_embed = create_success_embed("SNOW NODE VPS Created!", f"Your VPS has been successfully deployed by an admin!")
            add_field(dm_embed, "VPS Details", f"**VPS ID:** #{vps_count}\n**Container Name:** `{container_name}`\n**Configuration:** {config_str}\n**Status:** Running\n**Created:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", False)
            add_field(dm_embed, "Management", "• Use `!manage` to start/stop/reinstall your SNOW NODE VPS\n• Use `!manage` → SSH for terminal access\n• Contact SNOW NODE admin for upgrades or issues", False)
            add_field(dm_embed, "Important Notes", "• Full root access via SSH\n• Ubuntu 22.04 pre-installed\n• Back up your data regularly with SNOW NODE tools", False)
...
class ManageView(discord.ui.View):
        ...
        if len(vps_list) > 1:
            options = [
                discord.SelectOption(
                    label=f"SNOW NODE VPS {i+1} ({v.get('config', 'Custom')})",
                    description=f"Status: {v.get('status', 'unknown')}",
                    value=str(i)
                ) for i, v in enumerate(vps_list)
            ]
            self.select = discord.ui.Select(placeholder="Select a SNOW NODE VPS to manage", options=options)
            self.select.callback = self.select_vps
            self.add_item(self.select)
            self.initial_embed = create_embed("SNOW NODE VPS Management", "Select a VPS from the dropdown menu below.", 0x1a1a1a)
    ...
    async def create_vps_embed(self, index):
        ...
        embed = create_embed(
            f"SNOW NODE VPS Management - VPS {index + 1}",
        ...
        if suspended:
            add_field(embed, "⚠️ Suspended", "This SNOW NODE VPS is suspended. Contact an admin to unsuspend.", False)

        live_stats = f"**CPU Usage:** {cpu_usage}\n**Memory:** {memory_usage}\n**Disk:** {disk_usage}"
        add_field(embed, "📈 Live Usage", live_stats, False)

        add_field(embed, "🎮 Controls", "Use the buttons below to manage your SNOW NODE VPS", False)
    ...
    def add_action_buttons(self):
        if not self.is_shared and not self.is_admin:
            ...
            # Owner-only buttons
            optimize_button = discord.ui.Button(label="⚡ Optimize", style=discord.ButtonStyle.primary)
            optimize_button.callback = lambda inter: self.action_callback(inter, 'optimize')
            self.add_item(optimize_button)
            
            security_button = discord.ui.Button(label="🛡️ Security Scan", style=discord.ButtonStyle.primary)
            security_button.callback = lambda inter: self.action_callback(inter, 'security')
    ...
    async def select_vps(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This is not your SNOW NODE VPS!"), ephemeral=True)
            return
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if suspended and not self.is_admin and action != 'stats':
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This SNOW NODE VPS is suspended. Contact an admin to unsuspend."), ephemeral=True)
            return
        ...
        if action == 'stats':
            ...
            memory_usage = await get_container_memory(container_name)
            disk_usage = await get_container_disk(container_name)
            stats_embed = create_info_embed("📈 SNOW NODE Live Statistics", f"Real-time stats for `{container_name}`")
        ...
        if action == 'optimize':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the SNOW NODE VPS owner can optimize!"), ephemeral=True)
                return
                
            await interaction.response.defer(ephemeral=True)
            try:
                # Basic system optimization commands
                await execute_lxc(f"lxc exec {container_name} -- sudo apt-get update -y")
                await execute_lxc(f"lxc exec {container_name} -- sudo apt-get upgrade -y")
                await execute_lxc(f"lxc exec {container_name} -- sudo apt-get autoremove -y")
                await execute_lxc(f"lxc exec {container_name} -- sudo apt-get clean")
                
                await interaction.followup.send(embed=create_success_embed("Optimization Complete", f"SNOW NODE VPS `{container_name}` has been optimized!"), ephemeral=True)
            except Exception as e:
                await interaction.followup.send(embed=create_error_embed("Optimization Failed", str(e)), ephemeral=True)
            return

        if action == 'security':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the SNOW NODE VPS owner can run security scan!"), ephemeral=True)
                return
                
            await interaction.response.defer(ephemeral=True)
            try:
                # Basic security checks
                await execute_lxc(f"lxc exec {container_name} -- sudo apt-get install fail2ban -y")
                await execute_lxc(f"lxc exec {container_name} -- sudo systemctl enable fail2ban")
                await execute_lxc(f"lxc exec {container_name} -- sudo systemctl start fail2ban")
                
                # Check for open ports
                proc = await asyncio.create_subprocess_exec(
                    "lxc", "exec", container_name, "--", "ss", "-tuln",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                stdout, _ = await proc.communicate()
                open_ports = stdout.decode()
                
                security_embed = create_info_embed("🛡️ Security Scan Results", f"Security scan for `{container_name}`")
                add_field(security_embed, "Actions Taken", "• Installed fail2ban\n• Enabled fail2ban service", False)
                add_field(security_embed, "Open Ports", f"```\n{open_ports[:500]}\n```", False)
                
                await interaction.followup.send(embed=security_embed, ephemeral=True)
            except Exception as e:
                await interaction.followup.send(embed=create_error_embed("Security Scan Failed", str(e)), ephemeral=True)
            return

        if action == 'reinstall':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the SNOW NODE VPS owner can reinstall!"), ephemeral=True)
                return
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Cannot Reinstall", "Unsuspend the SNOW NODE VPS first."), ephemeral=True)
                return

            confirm_embed = create_warning_embed("SNOW NODE Reinstall Warning",
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        await interaction.followup.send(embed=create_info_embed("Recreating Container", f"Creating new SNOW NODE container `{self.container_name}`..."), ephemeral=True)
                        ...
                        save_data()
                        await interaction.followup.send(embed=create_success_embed("Reinstall Complete", f"SNOW NODE VPS `{self.container_name}` has been successfully reinstalled!"), ephemeral=True)
            ...
            try:
                ...
                vps["status"] = "running"
                save_data()
                await interaction.followup.send(embed=create_success_embed("VPS Started", f"SNOW NODE VPS `{container_name}` is now running!"), ephemeral=True)
                new_embed = await self.create_vps_embed(self.selected_index)
            ...
            try:
                ...
                vps["status"] = "stopped"
                save_data()
                await interaction.followup.send(embed=create_success_embed("VPS Stopped", f"SNOW NODE VPS `{container_name}` has been stopped!"), ephemeral=True)
                new_embed = await self.create_vps_embed(self.selected_index)
                await interaction.message.edit(embed=new_embed, view=self)
            except Exception as e:
            ...
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Cannot access suspended SNOW NODE VPS."), ephemeral=True)
                return
            await interaction.response.send_message(embed=create_info_embed("SSH Access", "Generating SNOW NODE SSH connection..."), ephemeral=True)
            ...
            try:
                ...
                if check_proc.returncode != 0:
                    ...
                    await execute_lxc(f"lxc exec {container_name} -- sudo apt-get update -y")
                    await execute_lxc(f"lxc exec {container_name} -- sudo apt-get install tmate -y")
                    await interaction.followup.send(embed=create_success_embed("Installed", "SNOW NODE SSH service installed!"), ephemeral=True)

                # Start tmate with unique session name using timestamp
                session_name = f"snow-node-session-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                if ssh_url:
                    try:
                        ssh_embed = create_embed("🔑 SNOW NODE SSH Access", f"SSH connection for VPS `{container_name}`:", 0x00ff88)
                        ...
                        await interaction.followup.send(embed=create_success_embed("SSH Sent", f"Check your DMs for SNOW NODE SSH link! Session: {session_name}"), ephemeral=True)
                    except discord.Forbidden:
                        await interaction.followup.send(embed=create_error_embed("DM Failed", "Enable DMs to receive SNOW NODE SSH link!"), ephemeral=True)
                else:
...
async def manage_vps(ctx, user: discord.Member = None):
    """Manage your SNOW NODE VPS or another user's VPS (Admin only)"""
    ...
    if user:
        ...
        if user_id_check != str(MAIN_ADMIN_ID) and user_id_check not in admin_data.get("admins", []):
            await ctx.send(embed=create_error_embed("Access Denied", "Only SNOW NODE admins can manage other users' VPS."))
            return
        
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await ctx.send(embed=create_error_embed("No VPS Found", f"{user.mention} doesn't have any SNOW NODE VPS."))
            return
        
        # Create admin view for managing another user's VPS
        view = ManageView(str(ctx.author.id), vps_list, is_admin=True, owner_id=user_id)
        await ctx.send(embed=create_info_embed(f"Managing {user.name}'s SNOW NODE VPS", f"Managing VPS for {user.mention}"), view=view)
    else:
        # User managing their own VPS
        user_id = str(ctx.author.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_embed("No VPS Found", "You don't have any SNOW NODE VPS. Contact an admin to create one.", 0xff3366)
            add_field(embed, "Quick Actions", "• `!manage` - Manage VPS\n• Contact SNOW NODE admin for VPS creation", False)
            await ctx.send(embed=embed)
            return
...
async def list_all_vps(ctx):
    """List all SNOW NODE VPS and user information (Admin only)"""
    total_vps = 0
    ...
    for user_id, vps_list in vps_data.items():
        try:
            ...
            # User summary
            user_summary.append(f"**{user.name}** ({user.mention}) - {user_vps_count} SNOW NODE VPS ({user_running} running, {user_suspended} suspended)")
            ...
            vps_info.append(f"❓ Unknown User ({user_id}) - {len(vps_list)} SNOW NODE VPS")
    
    # Create multiple embeds if needed to avoid character limit
    embeds = []
    
    # First embed with overview
    embed = create_embed("All SNOW NODE VPS Information", "Complete overview of all SNOW NODE VPS deployments and user statistics", 0x1a1a1a)
    add_field(embed, "System Overview", f"**Total Users:** {total_users}\n**Total VPS:** {total_vps}\n**Running:** {running_vps}\n**Stopped:** {stopped_vps}\n**Suspended:** {suspended_vps}", False)
    embeds.append(embed)
    
    # User summary embed
    if user_summary:
        embed = create_embed("SNOW NODE User Summary", f"Summary of all users and their SNOW NODE VPS", 0x1a1a1a)
    ...
    if vps_info:
        ...
        for i in range(0, len(vps_info), 15):
            ...
            embed = create_embed(f"SNOW NODE VPS Details ({i+1}-{min(i+15, len(vps_info))})", "List of all SNOW NODE VPS deployments", 0x1a1a1a)
...
async def manage_shared_vps(ctx, owner: discord.Member, vps_number: int):
    """Manage a shared SNOW NODE VPS"""
    owner_id = str(owner.id)
    user_id = str(ctx.author.id)
    if owner_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[owner_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or owner doesn't have a SNOW NODE VPS."))
        return
    vps = vps_data[owner_id][vps_number - 1]
    if user_id not in vps.get("shared_with", []):
        await ctx.send(embed=create_error_embed("Access Denied", "You do not have access to this SNOW NODE VPS."))
        return
...
async def share_user(ctx, shared_user: discord.Member, vps_number: int):
    """Share SNOW NODE VPS access with another user"""
    user_id = str(ctx.author.id)
    shared_user_id = str(shared_user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a SNOW NODE VPS."))
        return
    ...
    if shared_user_id in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Already Shared", f"{shared_user.mention} already has access to this SNOW NODE VPS!"))
        return
    vps["shared_with"].append(shared_user_id)
    save_data()
    await ctx.send(embed=create_success_embed("VPS Shared", f"SNOW NODE VPS #{vps_number} shared with {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("SNOW NODE VPS Access Granted", f"You have access to VPS #{vps_number} from {ctx.author.mention}. Use `!manage-shared {ctx.author.mention} {vps_number}`", 0x00ff88))
    except discord.Forbidden:
...
async def revoke_share(ctx, shared_user: discord.Member, vps_number: int):
    """Revoke shared SNOW NODE VPS access"""
    user_id = str(ctx.author.id)
    shared_user_id = str(shared_user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a SNOW NODE VPS."))
    ...
    if shared_user_id not in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Not Shared", f"{shared_user.mention} doesn't have access to this SNOW NODE VPS!"))
        return
    vps["shared_with"].remove(shared_user_id)
    save_data()
    await ctx.send(embed=create_success_embed("Access Revoked", f"Access to SNOW NODE VPS #{vps_number} revoked from {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("SNOW NODE VPS Access Revoked", f"Your access to VPS #{vps_number} by {ctx.author.mention} has been revoked.", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    """Delete a user's SNOW NODE VPS (Admin only)"""
    user_id = str(user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or user doesn't have a SNOW NODE VPS."))
        return
    vps = vps_data[user_id][vps_number - 1]
    container_name = vps["container_name"]

    await ctx.send(embed=create_info_embed("Deleting SNOW NODE VPS", f"Removing VPS #{vps_number}..."))
        ...
        if not vps_data[user_id]:
            ...
            if ctx.guild:
                ...
                if vps_role and vps_role in user.roles:
                    try:
                        await user.remove_roles(vps_role, reason="No SNOW NODE VPS ownership")
                    except discord.Forbidden:
                        logger.warning(f"Failed to remove SNOW NODE VPS role from {user.name}")
        save_data()

        embed = create_success_embed("SNOW NODE VPS Deleted Successfully")
        add_field(embed, "Owner", user.mention, True)
...
async def add_resources(ctx, vps_id: str, ram: int = None, cpu: int = None, disk: int = None):
    """Add resources to a SNOW NODE VPS (Admin only)"""
    ...
    if not found_vps:
        await ctx.send(embed=create_error_embed("VPS Not Found", f"No SNOW NODE VPS found with ID: `{vps_id}`"))
        return
    
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping SNOW NODE VPS `{vps_id}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("Resources Added", f"Successfully added resources to SNOW NODE VPS `{vps_id}`")
...
async def admin_add(ctx, user: discord.Member):
    """Add SNOW NODE admin (Main admin only)"""
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Already Admin", "This user is already the main SNOW NODE admin!"))
        return

    if user_id in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Already Admin", f"{user.mention} is already a SNOW NODE admin!"))
        return
    ...
    save_data()
    await ctx.send(embed=create_success_embed("Admin Added", f"{user.mention} is now a SNOW NODE admin!"))
    try:
        await user.send(embed=create_embed("🎉 SNOW NODE Admin Role Granted", f"You are now a SNOW NODE admin by {ctx.author.mention}", 0x00ff88))
    except discord.Forbidden:
...
async def admin_remove(ctx, user: discord.Member):
    """Remove SNOW NODE admin (Main admin only)"""
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Cannot Remove", "You cannot remove the main SNOW NODE admin!"))
        return

    if user_id not in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Not Admin", f"{user.mention} is not a SNOW NODE admin!"))
        return

    admin_data["admins"].remove(user_id)
    save_data()
    await ctx.send(embed=create_success_embed("Admin Removed", f"{user.mention} is no longer a SNOW NODE admin!"))
    try:
        await user.send(embed=create_embed("⚠️ SNOW NODE Admin Role Revoked", f"Your admin role was removed by {ctx.author.mention}", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def admin_list(ctx):
    """List all SNOW NODE admins (Main admin only)"""
    admins = admin_data.get("admins", [])
    main_admin = await bot.fetch_user(MAIN_ADMIN_ID)

    embed = create_embed("👑 SNOW NODE Admin Team", "Current SNOW NODE administrators:", 0x1a1a1a)
    ...
    if admins:
        ...
        add_field(embed, "🛡️ Admins", "No additional SNOW NODE admins", False)
...
async def user_info(ctx, user: discord.Member):
    """Get detailed information about a SNOW NODE user (Admin only)"""
    user_id = str(user.id)

    # Get user's VPS
    vps_list = vps_data.get(user_id, [])

    embed = create_embed(f"SNOW NODE User Information - {user.name}", f"Detailed information for {user.mention}", 0x1a1a1a)
    ...
    if vps_list:
        ...
        add_field(embed, "🖥️ SNOW NODE VPS Information", vps_summary, False)
        ...
        if len(vps_info) > 10:
            ...
            for i in range(10, len(vps_info), 10):
                ...
                additional_embed = create_embed(f"SNOW NODE VPS List ({i+1}-{min(i+10, len(vps_info))})", f"More VPS for {user.mention}", 0x1a1a1a)
        ...
        add_field(embed, "🖥️ SNOW NODE VPS Information", "**No VPS owned**", False)
        await ctx.send(embed=embed)

    # Check if user is admin
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", [])
    add_field(embed, "🛡️ SNOW NODE Admin Status", f"**{'Yes' if is_admin_user else 'No'}**", False)

@bot.command(name='serverstats')
@is_admin()
async def server_stats(ctx):
    """Show SNOW NODE server statistics (Admin only)"""
    total_users = len(vps_data)
    total_vps = sum(len(vps_list) for vps_list in vps_data.values())
    ...
    embed = create_embed("📊 SNOW NODE Server Statistics", "Current SNOW NODE server overview", 0x1a1a1a)
...
async def vps_info(ctx, container_name: str = None):
    """Get detailed SNOW NODE VPS information (Admin only)"""
    if not container_name:
        ...
        for user_id, vps_list in vps_data.items():
            try:
                ...
                for i, vps in enumerate(vps_list):
                    ...
                    if vps.get('suspended', False):
                        status_text += " (SUSPENDED)"
                    all_vps.append(f"**{user.name}** - SNOW NODE VPS {i+1}: `{vps['container_name']}` - {status_text}")
            except:
                pass
        ...
        for i in range(0, len(all_vps), 20):
            ...
            embed = create_embed(f"🖥️ All SNOW NODE VPS ({i+1}-{min(i+20, len(all_vps))})", f"List of all SNOW NODE VPS deployments", 0x1a1a1a)
            add_field(embed, "VPS List", "\n".join(chunk), False)
            await ctx.send(embed=embed)
    else:
        ...
        if not found_vps:
            await ctx.send(embed=create_error_embed("VPS Not Found", f"No SNOW NODE VPS found with container name: `{container_name}`"))
            return

        suspended_text = " (SUSPENDED)" if found_vps.get('suspended', False) else ""
        embed = create_embed(f"🖥️ SNOW NODE VPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}", 0x1a1a1a)
...
async def restart_vps(ctx, container_name: str):
    """Restart a SNOW NODE VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Restarting VPS", f"Restarting SNOW NODE VPS `{container_name}`..."))
    ...
    try:
        ...
        await ctx.send(embed=create_success_embed("VPS Restarted", f"SNOW NODE VPS `{container_name}` has been restarted successfully!"))
...
async def backup_vps(ctx, container_name: str):
    """Create a snapshot of a SNOW NODE VPS (Admin only)"""
    snapshot_name = f"snow-node-{container_name}-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}"

    await ctx.send(embed=create_info_embed("Creating SNOW NODE Backup", f"Creating snapshot of `{container_name}`..."))

    try:
        await execute_lxc(f"lxc snapshot {container_name} {snapshot_name}")
        await ctx.send(embed=create_success_embed("Backup Created", f"SNOW NODE Snapshot `{snapshot_name}` created successfully!"))
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    """Restore a SNOW NODE VPS from snapshot (Admin only)"""
    await ctx.send(embed=create_info_embed("Restoring VPS", f"Restoring `{container_name}` from SNOW NODE snapshot `{snapshot_name}`..."))

    try:
        await execute_lxc(f"lxc restore {container_name} {snapshot_name}")
        await ctx.send(embed=create_success_embed("VPS Restored", f"SNOW NODE VPS `{container_name}` has been restored from snapshot!"))
...
async def list_snapshots(ctx, container_name: str):
    """List all snapshots for a SNOW NODE VPS (Admin only)"""
    try:
        ...
        if snapshots:
            ...
            for i in range(0, len(snapshots), 20):
                ...
                embed = create_embed(f"📸 SNOW NODE Snapshots for {container_name} ({i+1}-{min(i+20, len(snapshots))})", f"List of snapshots", 0x1a1a1a)
                add_field(embed, "Snapshots", "\n".join([f"• {snap}" for snap in chunk]), False)
                await ctx.send(embed=embed)
        else:
            await ctx.send(embed=create_info_embed("No Snapshots", f"No SNOW NODE snapshots found for `{container_name}`"))
...
async def execute_command(ctx, container_name: str, *, command: str):
    """Execute a command inside a SNOW NODE VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Executing Command", f"Running command in SNOW NODE VPS `{container_name}`..."))
...
async def stop_all_vps(ctx):
    """Stop all SNOW NODE VPS using lxc stop --all --force (Admin only)"""
    await ctx.send(embed=create_warning_embed("Stopping All SNOW NODE VPS", "⚠️ **WARNING:** This will stop ALL running VPS on the SNOW NODE server.\n\nThis action cannot be undone. Continue?"))
    ...
    class ConfirmView(discord.ui.View):
        ...
        async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    embed = create_success_embed("All SNOW NODE VPS Stopped", f"Successfully stopped {stopped_count} VPS using `lxc stop --all --force`")
                    output_text = stdout.decode() if stdout else 'No output'
                    add_field(embed, "Command Output", f"```\n{output_text}\n```", False)
                    await interaction.followup.send(embed=embed)
                else:
                    error_msg = stderr.decode() if stderr else "Unknown error"
                    embed = create_error_embed("Stop Failed", f"Failed to stop SNOW NODE VPS: {error_msg}")
                    await interaction.followup.send(embed=embed)
        ...
        @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
        async def cancel(self, interaction: discord.Interaction, item: discord.ui.Button):
            await interaction.response.edit_message(embed=create_info_embed("Operation Cancelled", "The stop all SNOW NODE VPS operation has been cancelled."))
...
async def cpu_monitor_control(ctx, action: str = "status"):
    """Control SNOW NODE CPU monitoring system (Admin only)"""
    global cpu_monitor_active
    
    if action.lower() == "status":
        status = "Active" if cpu_monitor_active else "Inactive"
        embed = create_embed("SNOW NODE CPU Monitor Status", f"SNOW NODE CPU monitoring is currently **{status}**", 0x00ccff if cpu_monitor_active else 0xffaa00)
        add_field(embed, "Threshold", f"{CPU_THRESHOLD}% CPU usage", True)
        add_field(embed, "Check Interval", f"60 seconds (host)", True)
        await ctx.send(embed=embed)
    elif action.lower() == "enable":
        cpu_monitor_active = True
        await ctx.send(embed=create_success_embed("CPU Monitor Enabled", "SNOW NODE CPU monitoring has been enabled."))
    elif action.lower() == "disable":
        cpu_monitor_active = False
        await ctx.send(embed=create_warning_embed("CPU Monitor Disabled", "SNOW NODE CPU monitoring has been disabled."))
    else:
...
async def resize_vps(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    """Resize SNOW NODE VPS resources (Admin only)"""
    if ram is None and cpu is None and disk is None:
    ...
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping SNOW NODE VPS `{container_name}` to apply resource changes..."))
        try:
            await execute_lxc(f"lxc stop {container_name}")
            found_vps['status'] = 'stopped'
            save_data()
    ...
    try:
        ...
        embed = create_success_embed("VPS Resized", f"Successfully resized resources for SNOW NODE VPS `{container_name}`")
        add_field(embed, "Changes Applied", "\n".join(changes), False)
        await ctx.send(embed=embed)
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    """Clone a SNOW NODE VPS (Admin only)"""
    if not new_name:
        # Generate a new name if not provided
        timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')
        new_name = f"snow-node-{container_name}-clone-{timestamp}"
    
    await ctx.send(embed=create_info_embed("Cloning VPS", f"Cloning SNOW NODE VPS `{container_name}` to `{new_name}`..."))
    ...
    try:
        ...
        embed = create_success_embed("VPS Cloned", f"Successfully cloned SNOW NODE VPS `{container_name}` to `{new_name}`")
...
async def migrate_vps(ctx, container_name: str, target_pool: str):
    """Migrate a SNOW NODE VPS to a different storage pool (Admin only)"""
    await ctx.send(embed=create_info_embed("Migrating VPS", f"Migrating SNOW NODE VPS `{container_name}` to storage pool `{target_pool}`..."))
    ...
    try:
        ...
        temp_name = f"snow-node-{container_name}-temp-{int(time.time())}"
        ...
        await ctx.send(embed=create_success_embed("VPS Migrated", f"Successfully migrated SNOW NODE VPS `{container_name}` to storage pool `{target_pool}`"))
...
async def vps_stats(ctx, container_name: str):
    """Show detailed resource usage statistics for a SNOW NODE VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Statistics", f"Collecting statistics for SNOW NODE VPS `{container_name}`..."))
    ...
    try:
        ...
        embed = create_embed(f"📊 SNOW NODE VPS Statistics - {container_name}", f"Resource usage statistics", 0x1a1a1a)
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    """Manage SNOW NODE VPS network settings (Admin only)"""
    ...
    try:
        if action.lower() == "list":
            ...
            if proc.returncode == 0:
                ...
                embed = create_embed(f"🌐 SNOW NODE Network Interfaces - {container_name}", "Network configuration", 0x1a1a1a)
            ...
            await ctx.send(embed=create_success_embed("Network Limited", f"Set SNOW NODE network limit to {value} for `{container_name}`"))
        
        elif action.lower() in ["add", "remove"]:
            await ctx.send(embed=create_info_embed("Not Implemented", f"SNOW NODE Network {action} is not yet implemented. Use list or limit for now."))
...
async def vps_processes(ctx, container_name: str):
    """Show running processes in a SNOW NODE VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Processes", f"Listing processes in SNOW NODE VPS `{container_name}`..."))
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"⚙️ SNOW NODE Processes - {container_name}", "Running processes", 0x1a1a1a)
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    """Show recent logs from a SNOW NODE VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Logs", f"Fetching last {lines} lines from SNOW NODE VPS `{container_name}`..."))
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"📋 SNOW NODE Logs - {container_name}", f"Last {lines} log lines", 0x1a1a1a)
...
async def suspend_vps(ctx, container_name: str, *, reason: str = "Admin action"):
    """Suspend a SNOW NODE VPS (Admin only)"""
    found = False
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if vps.get('status') != 'running':
                    await ctx.send(embed=create_error_embed("Cannot Suspend", "SNOW NODE VPS must be running to suspend."))
                    return
                try:
                    await execute_lxc(f"lxc stop {container_name}")
                    vps['status'] = 'suspended'
                ...
                try:
                    ...
                    embed = create_warning_embed("🚨 SNOW NODE VPS Suspended", f"Your VPS `{container_name}` has been suspended by an admin.\n\n**Reason:** {reason}\n\nContact a SNOW NODE admin to unsuspend.")
                    await owner.send(embed=embed)
                except Exception as dm_e:
                    logger.error(f"Failed to DM owner {uid}: {dm_e}")
                await ctx.send(embed=create_success_embed("VPS Suspended", f"SNOW NODE VPS `{container_name}` suspended. Reason: {reason}"))
                found = True
                break
        if found:
            break
    if not found:
        await ctx.send(embed=create_error_embed("Not Found", f"SNOW NODE VPS `{container_name}` not found."))

@bot.command(name='unsuspend-vps')
@is_admin()
async def unsuspend_vps(ctx, container_name: str):
    """Unsuspend a SNOW NODE VPS (Admin only)"""
    found = False
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if not vps.get('suspended', False):
                    await ctx.send(embed=create_error_embed("Not Suspended", "SNOW NODE VPS is not suspended."))
                    return
                try:
                    vps['suspended'] = False
                    vps['status'] = 'running'
                    await execute_lxc(f"lxc start {container_name}")
                    save_data()
                    await ctx.send(embed=create_success_embed("VPS Unsuspended", f"SNOW NODE VPS `{container_name}` unsuspended and started."))
                    found = True
                except Exception as e:
...
async def suspension_logs(ctx, container_name: str = None):
    """View SNOW NODE suspension logs (Admin only)"""
    if container_name:
        # Specific VPS
        found = None
        ...
        if not history:
            await ctx.send(embed=create_info_embed("No Suspensions", f"No SNOW NODE suspension history for `{container_name}`."))
            return
        embed = create_embed("SNOW NODE Suspension History", f"For `{container_name}`")
        text = []
        ...
        if not all_logs:
            await ctx.send(embed=create_info_embed("No Suspensions", "No SNOW NODE suspension events recorded."))
            return
        # Split into embeds
        for i in range(0, len(all_logs), 10):
            chunk = all_logs[i:i+10]
            embed = create_embed(f"SNOW NODE Suspension Logs ({i+1}-{min(i+10, len(all_logs))})", f"Global suspension events (newest first)")
            add_field(embed, "Events", "\n".join(chunk), False)
            await ctx.send(embed=embed)

@bot.command(name='devmode')
@is_main_admin()
async def dev_mode_control(ctx, action: str):
    """Control development mode (Main admin only)"""
    global dev_mode
    
    if action.lower() == "on":
        dev_mode = True
        await ctx.send(embed=create_warning_embed("Development Mode Enabled", "🛠 DEVELOPMENT MODE ON - Only main admin can run commands."))
    elif action.lower() == "off":
        dev_mode = False
        await ctx.send(embed=create_success_embed("Development Mode Disabled", "Development mode turned off. All users can use commands according to their permissions."))
    else:
        await ctx.send(embed=create_error_embed("Invalid Action", "Use: `!devmode <on|off>`"))

@bot.command(name='help')
@dev_mode_check()
async def show_help(ctx):
    """Show SNOW NODE help information"""
    user_id = str(ctx.author.id)
    ...
    embed = create_embed("📚 SNOW NODE Command Help - User Commands", "SNOW NODE VPS Manager Commands:", 0x1a1a1a)

    user_commands = [
        ("!ping", "Check SNOW NODE bot latency"),
        ("!uptime", "Show host uptime"),
        ("!myvps", "List your SNOW NODE VPS"),
        ("!manage [@user]", "Manage your VPS or another user's VPS (Admin only)"),
        ("!share-user @user <vps_number>", "Share SNOW NODE VPS access"),
        ("!share-ruser @user <vps_number>", "Revoke SNOW NODE VPS access"),
        ("!manage-shared @owner <vps_number>", "Manage shared SNOW NODE VPS")
    ]
    ...
    if is_user_admin:
        ...
        embed = create_embed("📚 SNOW NODE Command Help - Admin Commands", "SNOW NODE VPS Manager Commands:", 0x1a1a1a)

        admin_commands = [
            ("!lxc-list", "List all LXC containers"),
            ("!create <ram_gb> <cpu_cores> <disk_gb> @user", "Create custom SNOW NODE VPS"),
            ("!delete-vps @user <vps_number> <reason>", "Delete user's SNOW NODE VPS"),
            ("!add-resources <vps_id> [ram] [cpu] [disk]", "Add resources to a SNOW NODE VPS"),
            ("!resize-vps <container> [ram] [cpu] [disk]", "Resize SNOW NODE VPS resources"),
            ("!suspend-vps <container> [reason]", "Suspend a SNOW NODE VPS"),
            ("!unsuspend-vps <container>", "Unsuspend a SNOW NODE VPS"),
            ("!suspension-logs [container]", "View SNOW NODE suspension logs"),
            ("!userinfo @user", "Get detailed SNOW NODE user information"),
            ("!serverstats", "Show SNOW NODE server statistics"),
            ("!vpsinfo [container]", "Get SNOW NODE VPS information"),
            ("!list-all", "View all SNOW NODE VPS and user information"),
            ("!restart-vps <container>", "Restart a SNOW NODE VPS"),
            ("!backup-vps <container>", "Create SNOW NODE VPS snapshot"),
            ("!restore-vps <container> <snapshot>", "Restore from SNOW NODE snapshot"),
            ("!list-snapshots <container>", "List SNOW NODE VPS snapshots"),
            ("!exec <container> <command>", "Execute command in SNOW NODE VPS"),
            ("!stop-vps-all", "Stop all SNOW NODE VPS with lxc stop --all --force"),
            ("!cpu-monitor <status|enable|disable>", "Control SNOW NODE CPU monitoring system"),
            ("!clone-vps <container> [new_name]", "Clone a SNOW NODE VPS"),
            ("!migrate-vps <container> <pool>", "Migrate SNOW NODE VPS to storage pool"),
            ("!vps-stats <container>", "Show SNOW NODE VPS resource stats"),
            ("!vps-network <container> <action> [value]", "Manage SNOW NODE network"),
            ("!vps-processes <container>", "List SNOW NODE processes"),
            ("!vps-logs <container> [lines]", "Show SNOW NODE system logs"),
            ("!devmode <on|off>", "Control development mode (Main admin only)")
        ]
...
async def manage_typo(ctx):
    ...
    await ctx.send(embed=create_info_embed("Command Correction", "Did you mean `!manage`? Use the correct SNOW NODE command."))
...
async def stats_alias(ctx):
    ...
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) in admin_data.get("admins", []):
        ...
        await ctx.send(embed=create_error_embed("Access Denied", "This SNOW NODE command requires admin privileges."))
...
def load_purge_safe():
    ...
    try:
        import json
        with open('purge_safe.json','r') as f:
            purge_safe = json.load(f)
    except:
        purge_safe = []
...
def save_purge_safe():
    global purge_safe
    import json
    with open('purge_safe.json','w') as f:
        json.dump(purge_safe,f,indent=4)

# integrate into existing save_data if exists
# (safe fallback if save_data not found)
try:
    old_save = save_data
    def save_data():
        old_save()
...
@bot.command(name="purge-sf-add")
@is_admin()
async def purge_sf_add(ctx, container_id: str):
    global purge_safe
    if container_id in purge_safe:
        await ctx.send(f"`{container_id}` already purge-safe.")
        return
    purge_safe.append(container_id)
    save_purge_safe()
    await ctx.send(f"Added `{container_id}` to purge-safe list.")

@bot.command(name="purge-sf-remove")
@is_admin()
async def purge_sf_remove(ctx, container_id: str):
    global purge_safe
    if container_id not in purge_safe:
        await ctx.send(f"`{container_id}` is not in purge-safe list.")
        return
    purge_safe.remove(container_id)
    save_purge_safe()
    await ctx.send(f"Removed `{container_id}` from purge-safe list.")

@bot.command(name="purge-sf-list")
@is_admin()
async def purge_sf_list(ctx):
    global purge_safe
    if not purge_safe:
        await ctx.send("No purge-safe VPS.")
        return
    await ctx.send("**Purge Safe VPS:**
" + "\n".join(f"- {c}" for c in purge_safe))

@bot.command(name="purge-sf-clear")
@is_admin()
async def purge_sf_clear(ctx):
    global purge_safe
    purge_safe=[]
    save_purge_safe()
    await ctx.send("Purge-safe list cleared.")

@bot.command(name="purge")
@is_main_admin()
async def purge(ctx):
    global purge_safe, vps_data
    deleted=0
    skipped=0

    await ctx.send("⚠️ Purge started...")

    for user_id, vps_list in list(vps_data.items()):
        updated=[]
        for vps in vps_list:
            cname=vps.get("container_name")
            if cname in purge_safe:
                skipped+=1
                updated.append(vps)
                continue
            try:
                await execute_lxc(f"lxc delete {cname} --force")
                deleted+=1
            except:
                continue
        vps_data[user_id]=updated
        if not updated:
            try:
                del vps_data[user_id]
            except:
